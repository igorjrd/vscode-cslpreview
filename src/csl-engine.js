  ...
  buildPreviewContent(style) {
    try {
      ...
      for (let i = 0; i < this.citablesIds.length; i++) {
        ...
        individualCitations.push(
          this.proc.appendCitationCluster(citation, [], [])[0][1]
        );
      }
      let citationItems = [];
      for (let i = 0; i < this.citablesIds.length; i++)
        citationItems.push({ id: this.citablesIds[i] });
      let uniqueCitation = this.proc.processCitationCluster(
        { citationItems },
        [],
        []
      );
      let bibliography = formatBibliographyHtml(this.proc.makeBibliography());
      let previewContent = formatHtmlPreview(
        individualCitations,
        uniqueCitation,
        bibliography
      );
      ...
      console.error("buildPreviewContent error:", e);
...
class CiteprocSys {
  ...
  retrieveLocale(lang) {
    const langUrl = `https://asset.cdn.ivysci.com/csl/locales/locales-${lang}.xml`;
    const res = fetch(langUrl).text();
    return res;
  }
  retrieveItem(itemId) {
...
function formatBibliographyHtml(data) {
  ...
  if (data[0].hangingindent) {
    ...
    let numberStyled =
      '<div class="csl-left-margin" style="float:left; display:inline-block;' +
      offsetSpec +
      '">';
    ...
    if (data[0].maxoffset) {
      ...
      widthSpec = "width:" + 95 + "%;";
      let textStyled =
        '<div class="csl-right-inline" style="display: inline-block;white-space: normal;' +
        widthSpec +
        '">';
